...
### Changed
- Refreshed the vendored `scripts/schemas/geoparquet-2.0.0.schema.json` to match `opengeospatial/geoparquet@main`: [#301](https://github.com/opengeospatial/geoparquet/pull/301) dropped the stale "or 2D+M bbox (xmin, ymin, mmin, xmax, ymax, mmax)" wording from the 6-element `bbox` description, since [#278](https://github.com/opengeospatial/geoparquet/pull/278) had already removed that form from the prose. Six elements always mean XYZ. The vendored copy now differs from upstream by nothing; no fixture changes (the corpus has no XYM bbox and the constraint itself — `minItems`/`maxItems` 6 — is unchanged).
