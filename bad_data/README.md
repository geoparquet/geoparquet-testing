...
| `geometry-column-not-in-columns.parquet` | `centroid` is a GEOMETRY column but has no entry in `columns` (each geometry column MUST be included) | `geometry_column_undeclared` |
| `geometry-types-duplicate-entries.parquet` | geometry_types lists "Point" twice; the entries must be unique | `schema_validation_error` |
...
| `orientation-ccw-declared-hole-ccw.parquet` | orientation=counterclockwise declared but the interior ring is wound CCW (holes must be CW) | `orientation_mismatch` |
| `orientation-ccw-declared-multipolygon-part-cw.parquet` | orientation=counterclockwise declared but the second polygon of a MultiPolygon is wound CW | `orientation_mismatch` |
...
| `primary-column-not-in-columns.parquet` | `primary_column` value does not appear as a key in `columns` | `primary_column_mismatch` |
