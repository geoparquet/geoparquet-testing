...
| `epoch-on-unsupported-crs.parquet` | coordinate_epoch set on EPSG:4326 (static datum) which does not support it | `epoch_unsupported` |
...
| `version-1-0-with-2-0-features.parquet` | version=1.0.0 declared but file uses 2.0-only coordinate_epoch field | `version_feature_mismatch` |
