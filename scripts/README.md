...
that can emit that logical type. sedonadb is a heavy, generation-only dependency (a
~50 MB wheel needed by nobody else in the pipeline), so it is **not** part of
`scripts/pyproject.toml` / `uv.lock`; install it into a separate environment:
